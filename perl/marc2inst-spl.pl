...
sub make_hi {
  ...
  foreach my $item ($marc->field($itemtag)) {
    ...
    if (!$hseen->{$hkey}) {
      $hid = uuid($hkey);
      $hrec->{id} = $hid;
      $hrec->{hrid} = $bhrid . "-" . sprintf("%03d", $inc);
      $hrec->{instanceId} = $bid;
      $hrec->{permanentLocationId} = $loc;
      $hrec->{callNumber} = $item->subfield('e') || '';
    ...
    # make item record;
    my $irec = {};
    my $iid = $item->subfield('q');
    if ($iid) {
      $iid =~ s/^\.//;
      $irec->{id} = uuid($iid);
      $irec->{holdingsRecordId} = $hid;
      $irec->{hrid} = $iid;
      $irec->{barcode} = $item->subfield('b') || '';
      $irec->{permanentLoanTypeId} = $refdata->{loantypes}->{'Can circulate'};
      $irec->{materialTypeId} = $item->subfield('c') || '71fbd940-1027-40a6-8a48-49b44d795e46'; # defaulting to unspecified
      $irec->{status}->{name} = 'Unavailable';
      if ($item->subfield('h')) {
        my @notes = ("Price: " . $item->subfield('h'));
        foreach (@notes) {
