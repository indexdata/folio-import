...
This is a place to save scripts for loading (migrating) user (and inventory) records.  The project at FOLIO-FSE (https://github.com/FOLIO-FSE) doesn't seem to do everything we need.  There are some conversion scripts written in Phython but nothing that loads the converted records.  We may find it a little easier to use JS or Perl to accomplish our goals of converting and loading user data.

## Usage

### JavaScript

All JS scripts read server settings from config.js.  If config.js does not exist, the scripts will use config.default.js instead.
...
* authpath -- The path for login (eg. /bl-users/login, /authn/login)
* tenant -- Okapi tenant on the targeted sytem
* username -- (eg. diku_admin)
* password -- (eg. admin)
...
Copy the config.default.js file to config.js and set the okapi url to the targeted server.  Tenant, user and password are should also be set if not a diku setup.  The delay between actions may be set to 0 if you don't want a delay.  There are a bunch of saved configs for various popular servers in the config directory.
...
Since JS doesn't have any reliable MARC libraries, working with MARC records is best acheived using Perl. So far the scripts only deal with conversion-- nothing talks to okapi.

### loadCreds.js

Since mod-user-import does not create credentials records, this script is a way to do so.  It will read a file in "credentials" format and first check to see if the userId exists and then will POST the credential record to the /authn/credentials.  If a credentials record already exists for the userId, then the sript will return a detailed error message.
