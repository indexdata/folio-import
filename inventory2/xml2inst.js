...
try {
  if (!rawFile) { throw "Usage: node xml2inst.js <conf_file> <marcxml_file>" }
  ...
  let iprefix = (iconf) ? iconf.hridPrefix : '';
  ...
  const parser = new XMLParser(xoptions);
  
  let leftOvers = '';
  fileStream.on('data', (chunk) => {
    chunk = chunk.replace(/<(\/)?marc:/g, '<$1');
    chunk = leftOvers + chunk;
    let recs = chunk.match(/<record.+?<\/record>|<record.+$/sg);
    let lastRec = recs[recs.length - 1];
    if (!lastRec.match(/<\/record>/)) {
      leftOvers = lastRec;
      recs.pop();
    } else {
      leftOvers = '';
    ...
    for (let k = 0; k < recs.length; k++) {
      ...
      try { 
        let o = parser.parse(xmlStr);
        let r = o.record;
        // console.log(r);
        if (r.leader && r.leader['#text']) {
          marc.leader = r.leader['#text'];
        } else {
          marc.leader = r.leader;
        }
        if (r.controlfield) {
          r.controlfield.forEach(f => {
            let d = f['#text'];
            let t = f._tag;
            if (!marc.fields[t]) marc.fields[t] = [];
            marc.fields[t].push(d);
          });
        }
        if (r.datafield) {
          r.datafield.forEach(f => {
            let t = f._tag;
            console.log(f);
            if (f.subfield) {
              if (!marc.fields[t]) marc.fields[t] = [];
              let subs = [];
              if (f.subfield['#text']) {
                subs.push({ [f.subfield._code]: f.subfield['#text'] });
              } else {
                f.subfield.forEach(s => {
                  subs.push({ [s._code]: s['#text'] });
                });
              }
              marc.fields[t].push({ ind1: f._ind1, ind2: f._ind2, subfields: subs });
            }
          })
        }
      ...
      if (prefix && marc.fields['001']) {
        marc.fields['001'][0] = prefix + marc.fields['001'][0];
      }
