...
try {
  if (!rawFile) { throw "Usage: node marc2inst-stc.js <conf_file> <instance_level_csv_file> <raw_marc_file>" }
    ...
    try {
      ...
      robj[prop].forEach(p => {
        ...
        if (p.code) {
          ...
          let lcCode = p.code.toLowerCase();
          refData[prop][lcCode] = p.id;
        } 
        if (p.name) {
  ...
  let stream = fs.createReadStream(itemFile);
  ...
  for await (let line of rl) {
    ...
    if (lc === 1) {
      ...
      itemMap[r.CATKEY] = { s: r.CAT_SHADOW, cat: [ r.ITEM_CAT1, r.ITEM_CAT3 ] };
    }
  }
  // throw(itemMap);
  ...
  fileStream.on('data', (chunk) => {
    ...
    for (let k = 0; k < recs.length; k++) {
      ...
      if (inst.hrid) {
        ...
        let catKey = inst.hrid.replace(/^u/, '');
        let imap = itemMap[catKey];
        if (imap) {
          if (imap.s === '1') inst.discoverySuppress = true;
          imap.cat.forEach(c => {
            let lcCode = c.toLowerCase();
            let scodeId = refData.statisticalCodes[lcCode];
            if (scodeId) {
              if (!inst.statisticalCodeIds) inst.statisticalCodeIds = [];
              inst.statisticalCodeIds.push(scodeId);
            } else if (c) {
              console.log(`WARN no statistical code found for ${c}`);
            }
          });
        }
        inst.id = instId;
