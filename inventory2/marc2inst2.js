import { parseMarc, getSubs, mij2raw, fields2mij, getSubsHash } from '../js-marc/js-marc2.mjs';
import fs from 'fs';
...
const funcs = {
  ...
  set_subject_type_id: function (data, param) {
    let k = (param && param.name) ? param.name.toLowerCase() : 'type of entity unspecified';
    ...
    for (let k = 0; k < recs.length; k++) {
      ...
      if (marc.fields['004'] && conf.hasMfhd) {
        if (conf.hasMfhd) {
          for (let t in marc.fields) {
            if (t < '010') {
              for (let x = 0; x < marc.fields[t].length; x++) {
                marc.fields[t][x] = marc.fields[t][x].data;
              }
            }
          }
          ...
          let ctrl = (marc.fields['001']) ? marc.fields['001'][0].data : '(unknown)'
      ...
      if (conf.controlNum) {
        ...
        if (cnum) {
          ...
          let f001 = (marc.fields['001']) ? marc.fields['001'][0].data : '';
          if (f001) {
            let f003 = (marc.fields['003']) ? marc.fields['003'][0].data : '';
            let oldNum = f001;
            if (f003) {
      ...
      if (marc.fields['001']) marc.fields['001'][0].data = marc.fields['001'][0].data.trim();
      if (conf.callNumbers) {
      ...
      if (prefix && marc.fields['001']) {
        let newData = { data: prefix + marc.fields['001'][0].data, _: marc.fields['001'][0]._ };
        marc.updateField('001', newData);
      }
      let hrid = (marc.fields['001']) ? marc.fields['001'][0].data : '';
      if (!hrid) {
        ttl.errors++;
