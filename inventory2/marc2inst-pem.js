/*
  PEM needs an bespoke script to read a separate suppressed file.  This file should be called "suppressed.tsv" or else
...
try {
  ...
  let supp = (conf.suppress.tag) ? conf.suppress : '';
  ...
  const supMap = {};
  let supFile = wdir + '/suppressed.tsv';
  let supDat = fs.readFileSync(supFile, { encoding: 'utf8' });
  supDat.split(/\n/).forEach(l => {
    let c = l.split(/\t/);
    let k = c[0];
    supMap[k] = 1;
      ...
      if (inst.hrid) {
        ...
        if (inst.notes) {
          inst.notes.forEach(n => {
            n.staffOnly = n.staffOnly.replace(/ false.*/, '');
          });
        }
