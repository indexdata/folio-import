...
*/
  ...
  rl.on('line', l => {
    ...
    if (!l.match(/createdDate/)) {
      ...
      if (tap && tap[0]) {
        if (!u.customFields) u.customFields = {};
        u.customFields.tapid = tap[0];
      }
      u.personal = per;
  ...
  rl.on('close', () => {
    ...
    // if (tapFlag) inc.customFields = [{ refId: 'tapid' }]
    for (let n in depts) {
