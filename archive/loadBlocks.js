...
(async () => {
  try {
    ...
    let inData;
    if (!inFile) {
      throw new Error('Usage: node loadBlocks.js <blocks_file> [ <limit> ]');
    } else if (!fs.existsSync(inFile)) {
      throw new Error('Can\'t find input file');
    } else {
      inData = require(inFile);
      if (inData.manualblocks) {
        inData = inData.manualblocks;
      }
    }
    ...
    const actionUrl = config.okapi + '/manualblocks';
    ...
    for (let x = 0; x < limit; x++) {
      try {
        ...
        logger.info(`Successfully added block to ${inData[x].userId}`);
        success++
      } catch (e) {
        try {
          await superagent
          ...
          logger.info(`Successfully updated block for ${inData[x].userId}`);
          updated++
        } catch (e) {
